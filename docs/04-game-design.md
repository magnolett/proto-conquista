...
### Profundidade F5-lite (mundo grande + FFA — 2026-07-06, iteração 3)
- **Mundo 4×** (`CFG` 2560×1440) com **câmera** no cliente: scroll = zoom ancorado no cursor (1×–4×), botão do MEIO arrasta, **minimapa clicável** com o retângulo da viewport. Diais escalados junto (fleetSpeed 150 · SUPPLY.range 720 · zonas +50% · distW da IA −40% · 14 pares de neutras no 1v1).
- **FFA com até 3 IAs** (menu `[E]`, padrão 3): rivais extras `e2`/`e3` com persona, doutrina e relógio de decisão PRÓPRIOS; capitais nos cantos (1v1 mantém o espelho clássico). Vitória = eliminar TODOS os rivais ou dominar o centro; derrota imediata se o jogador cair — e **qualquer rival pode vencer por domínio**.
- **Hostilidade SIMÉTRICA (garantia de design):** a IA não tem conceito de "jogador" — o código só distingue *eu, neutros e rivais*, então agressão, defesa, all-in coordenado e doutrina reagem a qualquer rival, e a geometria de cantos cria fronteiras IA×IA naturais. **Medido** (difícil, 3 IAs, jogador passivo, 3 seeds): **22–49 ataques IA→IA e 8–19 capturas IA→IA por partida**. **Trava de regressão:** o teste "as IAs brigam ENTRE SI" (`packages/sim/src/ffa.test.ts`) quebra o CI se o FFA degenerar em "todas contra o jogador". Deixar duas IAs se desgastarem e atacar a sobrevivente é estratégia deliberadamente suportada.
